...
for machine, row in zip(data.keys(), range(1, num_machines + 1)):
    ...
    filtered_activity = data[machine]["filtered activity"]
    raw_activity = data[machine]["raw activity"]

    plt.plot(time, raw_activity, 'go', markersize=1)
    plt.plot(time, filtered_activity, 'b--')
